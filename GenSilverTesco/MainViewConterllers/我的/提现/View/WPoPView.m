...
@interface WPoPView () <UITableViewDelegate, UITableViewDataSource>
...
#pragma mark - Data
@property (nonatomic, copy) NSArray<WPoPaction *> *actions;
...
- (void)setStyle:(PopoverViewStyle)style {
    _style = style;
    _tableView.separatorColor = [WPoPViewCell bottomLineColorForStyle:_style];
...
- (CGFloat)calculateMaxWidth {
    ...
    UIFont *titleFont = [WPoPViewCell titleFont];
    for (WPoPaction *action in _actions) {
        imageWidth = 0.f;
        titleLeftEdge = 0.f;
...
- (void)showToView:(UIView *)pointView withActions:(NSArray<WPoPaction *> *)actions {
...
/*! @brief 指向指定的点来显示弹窗 */
- (void)showToPoint:(CGPoint)toPoint withActions:(NSArray<WPoPaction *> *)actions {
    _actions = [actions copy];
    // 计算箭头指向方向
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    WPoPViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kPopoverCellIdentifier];
    if (!cell) {
        cell = [[WPoPViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kPopoverCellIdentifier];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [UIView animateWithDuration:0.25f animations:^{
        ...
        WPoPaction *action = _actions[indexPath.row];
