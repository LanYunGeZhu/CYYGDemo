...
{
    if (self.noSelectedArray) {
        
        if ([self getUrlStingConunt] + self.selectedAssetArray.count >= self.maximumNumberOfSelection ) {
            NSString  *str = [NSString stringWithFormat:@"最多选择%lu张照片",(unsigned long)self.maximumNumberOfSelection];
            [JKPromptView showWithImageName:@"picker_alert_sigh" message:str];
            return;
        }
...
- (int)getUrlStingConunt{
    __block int x = 0;
    [self.noSelectedArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
